...
suite("test_cloud_alter_user", "account") {
    ...
    sql """create user test_auth_user1 identified by 'Cloud12345' default role 'test_auth_role1'"""
    ...
    sql """create user test_auth_user2 identified by 'Cloud12345' password_history default"""
    sql """grant all on *.* to test_auth_user2"""
    sql """alter user test_auth_user2 identified by 'Cloud12345'"""
    sql """set password for 'test_auth_user2' = password('Cloud12345')"""
    ...
    def result1 = connect(user = 'test_auth_user2', password = 'Cloud12345', url = context.config.jdbcUrl) {
        sql 'select 1'
    }

    sql """alter user test_auth_user2 password_history 2"""
    sql """alter user test_auth_user2 identified by 'Cloudabc12345'"""
    sql """alter user test_auth_user2 identified by 'Cloudabc123456'"""
    test {
        sql """alter user test_auth_user2 identified by 'Cloudabc12345'"""
        exception "Cannot use these credentials for 'default_cluster:test_auth_user2'@'%' because they contradict the password history policy"
    }
    result1 = connect(user = 'test_auth_user2', password = 'Cloudabc123456', url = context.config.jdbcUrl) {
        sql 'select 1'
    }
    sql """set global password_history=0""" // set to disabled

    // 3. test FAILED_LOGIN_ATTEMPTS and PASSWORD_LOCK_TIME
    sql """create user test_auth_user3 identified by 'Cloud12345' FAILED_LOGIN_ATTEMPTS 2 PASSWORD_LOCK_TIME 1 DAY"""
    sql """grant all on *.* to test_auth_user3"""
         
    // login success in multi times
    result1 = connect(user = 'test_auth_user3', password = 'Cloud12345', url = context.config.jdbcUrl) {
        sql 'select 1'
    }
    result1 = connect(user = 'test_auth_user3', password = 'Cloud12345', url = context.config.jdbcUrl) {
        sql 'select 1'
    ...
    try {
        connect(user = 'test_auth_user3', password = 'Cloudwrong123', url = context.config.jdbcUrl) {}
    ...
    try {
        connect(user = 'test_auth_user3', password = 'Cloud12345', url = context.config.jdbcUrl) {}
        assertTrue(false. "should not be able to login")
    } catch (Exception e) {
    ...
    // 5. text expire
    sql """create user test_auth_user4 identified by 'Cloud12345' PASSWORD_EXPIRE INTERVAL 5 SECOND"""
    sql """grant all on *.* to test_auth_user4"""
    result1 = connect(user = 'test_auth_user4', password = 'Cloud12345', url = context.config.jdbcUrl) {
        sql 'select 1'
    }
    sleep(6000)
    try {
        connect(user = 'test_auth_user4', password = 'Cloud12345', url = context.config.jdbcUrl) {}
        assertTrue(false. "should not be able to login")
    } catch (Exception e) {
    ...
    sql """create user test_auth_user4 identified by 'Cloud12345'"""
    sql """grant all on *.* to test_auth_user4"""
    result1 = connect(user = 'test_auth_user4', password = 'Cloud12345', url = context.config.jdbcUrl) {
        sql 'select 1'
    }
