...
#ifndef NDEBUG
    const IColumn::Offsets& offsets_data = get_offsets_data();
    const auto it =
...
void ColumnSparse::get_indices_of_non_default_rows(IColumn::Offsets& indices, size_t from,
                                                   size_t limit) const {
...
const IColumn::Offsets& ColumnSparse::get_offsets_data() const {
    return assert_cast<const ColumnUInt64&>(*offsets).get_data();
}
IColumn::Offsets& ColumnSparse::get_offsets_data() {
