...
Status StorageEngine::execute_task(EngineTask* task) {
    auto lock_related_tablets = [&]() -> std::vector<std::unique_lock<doris::SharedMutex>> {
        ...
        std::vector<std::unique_lock<doris::SharedMutex>> wrlocks;
        for (TabletInfo& tablet_info : tablet_infos) {
            ...
            if (tablet != nullptr) {
                ...
                wrlocks.push_back(std::unique_lock<doris::SharedMutex>(tablet->get_header_lock()));
            } else {
