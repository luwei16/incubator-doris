    ...
    if (is_scalar_type(column->type())) {
        ...
        *writer = std::move(writer_local);
        ...
        case FieldType::OLAP_FIELD_TYPE_ARRAY: {
            ...
            item_options.need_inverted_index = false;
...
Status ScalarColumnWriter::init() {
    ...
    if (_opts.need_inverted_index) {
        RETURN_IF_ERROR(InvertedIndexColumnWriter::create(
                ...
                _file_writer->path().parent_path().native(), _opts.inverted_index_analyser_type,
                _file_writer->fs()));
    }
    ...
    if (_opts.need_inverted_index) {
        ...
        if (writer != nullptr) {
            RETURN_IF_ERROR(InvertedIndexColumnWriter::create(
                    ...
                    _opts.inverted_index_analyser_type, writer->_file_writer->fs()));
        }
    }
    return Status::OK();
}
