...
Status InvertedIndexSearcherCache::prune() {
    ...
    auto pred = [curtime](const void* value) -> bool {
        ...
        if (cache_value->first_start_time != 0) {
            ...
            auto start_expired_time = cache_value->first_start_time + (config::index_searcher_cache_stale_sweep_time_sec / 2) * 1000;
        ...
        if (cache_value->last_visit_time != 0) {
            ...
            auto visit_expired_time = cache_value->last_visit_time + config::index_searcher_cache_stale_sweep_time_sec * 1000;
    ...
    if (prune_num > 0)
        LOG(INFO) << "prune " << prune_num
                  << " entries in index_searcher cache. cost(ms): " << watch.elapsed_time() / 1000 / 1000;
    return Status::OK();
}
...
void InvertedIndexSearcherCache::_insert(const InvertedIndexSearcherCache::CacheKey& key, CacheValue& value, InvertedIndexCacheHandle* handle) {
    auto deleter = [](const doris::CacheKey& key, void* value) {
        InvertedIndexSearcherCache::CacheValue* cache_value = (InvertedIndexSearcherCache::CacheValue*)value;
        delete cache_value;
    };

    auto lru_handle = _cache->insert(key.index_file_path, &value, sizeof(InvertedIndexSearcherCache::CacheValue),
