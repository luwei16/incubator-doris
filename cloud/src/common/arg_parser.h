...
/**
 * This class implements aguments parser, it accepts both short '-arg' and
 ...
 *     ArgParser::new_arg<long>       ("duration", 10086   ,    "duraiton for the test in seconds"),
...
public:
  typedef std::map<std::string, std::tuple<std::variant<std::string, long, double, bool>, std::string>> arg_map_t;
  typedef arg_map_t::value_type arg_t;
...
public:
   ...
   */
  template<typename T>
  static arg_t new_arg(const std::string& name, const T& val,
...
public:
  ...
  ArgParser(const arg_map_t& args = {}, bool skip_unknown_arg = false):
      args_(args),
      skip_unknown_(skip_unknown_arg)
  { }
   ...
   *      call to this function does not make sence
   * @return error msg that encountered
   */
  std::string parse(
      int argc,  char const* const* argv, arg_map_t* out = nullptr) {

    arg_map_t ret;
    // copy
    if (out != nullptr) ret = *out; 
    else ret = args_;
    ...
    for (int i = 0; i < argc; ++i) {
      ...
      if (arg.size() < 4 || arg[0] != '-' || arg[2] == '-' || eq == std::string::npos) {
        ...
        msg += "invalid arg: " + arg + "; ";
        if (skip_unknown_) continue; else return msg;
      }
    ...
    if (out != nullptr) *out = std::move(ret);
    else args_ = std::move(ret);
