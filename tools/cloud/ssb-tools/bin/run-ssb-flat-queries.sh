...
will use mysql client to connect Doris server which parameter is specified in 'selectdb-cluster.conf' file.
Usage: $0 
  "
    exit 1
}
...
run_sql() {
    echo "$@"
    mysql -h"${HOST}" -P"${QUERY_PORT}" -u"${USER}" -D"${DB}" -e "$@"
}
...
for i in '1.1' '1.2' '1.3' '2.1' '2.2' '2.3' '3.1' '3.2' '3.3' '3.4' '4.1' '4.2' '4.3'; do
    ...
    mysql -h"${HOST}" -P"${QUERY_PORT}" -u"${USER}" -D "${DB}" <"${QUERIES_DIR}/q${i}.sql" >/dev/null 2>&1
    # Then run 3 times and takes the average time
    res=$(mysqlslap -h"${HOST}" -P"${QUERY_PORT}" -u"${USER}" --create-schema="${DB}" --query="${QUERIES_DIR}/q${i}.sql" -F '\r' -i 3 | sed -n '2p' | cut -d ' ' -f 9,10)
    echo "q${i}: ${res}"
