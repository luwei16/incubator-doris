...
usage() {
    echo "
This script is used to create SSB tables, 
will use mysql client to connect Doris server which is specified in conf/selectdb-cluster.conf file.
Usage: $0 
  "
    exit 1
}
...
mysql -h"${HOST}" -u"${USER}" -P"${QUERY_PORT}" -e "CREATE DATABASE IF NOT EXISTS ${DB}"

echo "Run DDL from ${SSB_DDL}"
mysql -h"${HOST}" -u"${USER}" -P"${QUERY_PORT}" -D"${DB}" <"${SSB_DDL}"

echo "Run DDL from ${SSB_FLAT_DDL}"
mysql -h"${HOST}" -u"${USER}" -P"${QUERY_PORT}" -D"${DB}" <"${SSB_FLAT_DDL}"
